...
# Notes: Start with a reset command below:
# /system reset-configuration no-defaults=yes skip-backup=yes run-after-reset=router-config.rsc
#######################################
...
/interface wifi set wifi2 channel=ch-2ghz configuration=common-conf disabled=no 
/interface wifi set wifi1 channel=ch-5ghz configuration=common-conf disabled=no
...
add chain=input action=accept comment="Allow Estab & Related" connection-state=established,related,untracked
...
add action=accept chain=forward comment="LAN to WG" out-interface=wireguard1 in-interface-list=LAN
...
/ip service disable telnet,ftp,api,ssh,api-ssl
