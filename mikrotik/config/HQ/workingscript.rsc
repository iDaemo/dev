# 2023-07-10 14:47:37 by RouterOS 7.10.1
# software id = 04H0-FDC6
#
# model = CCR2004-16G-2S+
# serial number = HD0084APN5W
/interface bridge
add igmp-snooping=yes igmp-version=3 name=BRI-TEST protocol-mode=none \
    query-interval=30s vlan-filtering=yes
/interface ethernet
...
add bridge=BRI-TEST fast-leave=yes frame-types=admit-only-vlan-tagged \
    interface=ether15 trusted=yes
...
add action=accept chain=input dst-port=53,13231 protocol=udp
add action=accept chain=input dst-port=443,8291 protocol=tcp
add action=accept chain=input in-interface-list=VLANS
...
add action=jump chain=forward comment="allow internet traffic" \
    in-interface-list=VLANS jump-target=ChainForwardAll out-interface-list=\
    WAN
add action=jump chain=forward in-interface-list=WAN jump-target=\
    ChainForwardAll out-interface-list=VLANS
add action=jump chain=forward in-interface=wg-sonoshq jump-target=\
    ChainForwardAll out-interface-list=VLANS
add action=jump chain=forward in-interface-list=VLANS jump-target=\
    ChainForwardAll out-interface=wg-sonoshq
